...
в веб-разработке. Я расскажу вам о самых важных из них, и вы точно влюбитесь!
...
и не присматривались к [Node.js][1]. Ну или ваш шеф пока не одобряет этого.
Что ж, пора присмотреться — есть много отличных разумных доводов, чтобы выбрать Node.
Вот десять из них.
...
постоянно преобразовывать 1) логику в своей голове из JavaScript на серверный
фреймворк и 2) HTTP-запросы из JSON в серверные объекты.

Используя JavaScript во всём приложении, вы не только сбережёте разум, но и 
выиграете в скорости разработки. А переиспользуя модели и шаблоны
делаете приложение меньше, что снижает его сложность и уменьшает вероятность ошибок.

JavaScript завоёвывает мир, вряд ли тут что-то изменится в ближайшее время. JavaScript
работает на каждом компьютере мира и, похоже, так будет продолжаться и дальше.
...
Помимо сверхзвуковой скорости, еще одно настоящее чудо — событийный
...
параллельные процессы для выполнения работы. Но асинхронность требует
большого количества памяти и имеет известные сложности в реализации
с точки зрения программирования. К счастью, с Node.js всё проще: когда 
приложению нужно выполнить операцию ввода-вывода, оно посылает асинхронную 
задачу в событийный цикл вместе с колбеком и продолжает выполнять остальную часть программы.
...
избегая конфликтов версий. Но, кроме того, он может устанавливать глобальные
shell-команды и компилировать зависимые от платформы бинарные файлы.
Не припомню случая, когда npm заставлял меня задумываться — «почему эти
...
Но у меня есть хорошие новости. Если вы используете объектную базу данных 
вроде [Mongo][10], то и здесь найдется место для JavaScript.

С Node.js вы можете писать на одном языке код клиента, сервера
и базы данных. Просто храните данные в обычном JSON-формате везде —
...
с необходимостью обслуживать большое количество пользователей одновременно,
а с использованием веб-сокетов работа в реальном времени становится еще лучше.  
Двусторонние каналы передачи данных в веб-сокетах помогают отправлять данные
...
[Socket.io][11] — одна из наиболее популярных библиотек для работы с веб-сокетами, 
делающая разработку коллективных веб-приложений невероятно простой. Вот небольшой
сервер на socket.io:
...
Обычно веб-фреймворки работают с HTTP-запросами и ответами как с объектами.
...
Node может записывать и считывать потоки прямо в веб-сокеты или из них. То же
...
[Meteor][13]. Meteor — это веб-фреймворк нового поколения, написанный на Node,
с общим кодом для клиента и сервера. С ним вы можете сохранять данные в базе
прямо из вашего javascript-кода в браузере, а уже оттуда эти данные попадают
на сервер. В обратную сторону это тоже работает! Любые обновления на сервере
автоматически отсылаются клиенту. И даже больше: любая веб-страница, на которой 
...
Платформа Node.js завоевала сердца, и теперь для нее появляется все больше хостингов.
...
Но погодите, это ещё не всё! В любой платформе и продукте, с открытым кодом
или нет, большую роль играет сообщество. Здесь Node.js стяжал себе лавры —
...
стыдно за вопрос в IRC-чате или на Github. Сообщество очень активно: 
в `npm` уже больше 91,000 модулей. И ещё, сообщество не скупится на пожертвования, и в 2013 
году [было собрано больше $70,000][16] для оплаты публичных npm-серверов.
